...
This project is not the first computational approach to the Ur III corpus, and situating it honestly requires naming its predecessors. The most direct antecedent is Jaworski (2008), who built a semantic-grammar system over a ontologically modeled branch of the Ur III economic corpus, translating documents into a meaning-representation language — rule-based extraction in the same spirit as this project, though scoped to a selected activity domain rather than the full export. The largest-scale prior effort is the MTAAC project ("Machine Translation and Automated Analysis of Cuneiform Languages," 2017–2019; Pagé-Perron et al. 2017; Chiarcos et al. 2018), which took the same CDLI Ur III administrative corpus (~68,000 texts in its snapshot) as a testbed for machine translation, morphological annotation, named-entity work, and linked-open-data publication. Social-network analysis of cuneiform archives is likewise established: Anderson's dissertation (2017) and Bamman, Anderson, and Smith (2013) built and analyzed merchant networks from roughly 6,000 Old Assyrian texts from Kültepe-Kanesh, confronting the same name-disambiguation problems this project handles with patronymic anchoring. On the Assyriological side, Liu (2021) provides a quantitative-prosopographic study of ~6,000 Puzriš-Dagan tablets under Amar-Suen, reconstructing the office structure of the very archive that dominates this project's dated record, and Widell (2003) classified the Ur III texts of Ur by administrative function.

Against that background, this project's contribution is a combination rather than an invention: (i) extraction over the *full* CDLI Ur III export rather than a domain or archive subset; (ii) a deterministic, regression-tested rule base whose every decision is inspectable, in deliberate contrast to the statistical NLP lineage of MTAAC; and (iii) validation of extracted quantities against the scribes' own recorded totals. I have not found the šunigin-reconciliation technique in the literature surveyed, but that survey was conducted at the level of titles, abstracts, and summaries rather than full texts (Section 10, limitation 6), so this paper claims the technique's usefulness, not its novelty.
...
**Two extraction shapes, one scribal reality.** A point of architecture matters enough for the findings that it belongs here rather than in a footnote. Ur III scribes wrote attribution at two scopes. Bilateral receipts state both parties for a single transfer ("from X; Y received"). List-format documents — ration rolls, allocation registers — state the issuer once, in a header or footer, and let it govern every line item between; the recipients are the per-line names. The pipeline mirrors this with two coordinated outputs: a *flat* path that emits one transaction per quantity, inheriting any attribution found within the same accounting section, and a *hierarchical* path (tablet → record → entry) that preserves list structure, attaching the issuer at record level and each recipient at entry level. Neither path is stateless — attribution inherits within a section, and the hierarchy exists precisely to capture header-scoped attribution over list items — but the two paths have different coverage, and an earlier version of this project's network analysis used only the flat path, silently discarding the list-format relational data. Section 9 reports the corrected, combined analysis. One genuine statefulness limitation remains and is recorded in Section 10: attribution does not inherit *across* section boundaries, so a tablet-final issuer clause governing several totaled groups above it is credited only to its own section.
...
| Output | Count |
|---|---|
| Transactions extracted (flat path) | 254,133 |
| Tablets yielding ≥1 transaction | 56,906 (42%) |
| Flat transactions with issuer *and* recipient resolved | 36,851 (14.5%) |
| Hierarchical line-entries (list-format documents) | 274,113 |
| Hierarchical entries with named recipient | 89,479 |
| Hierarchical records with named issuer | 42,743 |
...
### 9.1 Construction, and a correction

The network reported here combines both extraction shapes described in Section 3. The flat bilateral path contributes 10,170 barley transactions with both parties named. The hierarchical path contributes 11,454 additional barley issuer→recipient pairs, formed by joining each record's header-scoped issuer to each of its entry-level recipients — exactly the relational structure of a ration roll, where the scribe wrote the issuing office once and the recipients line by line. After deduplication (a receipt can appear in both paths), the combined relational dataset comprises 11,839 distinct tablet-level attestations, yielding a directed network of **4,209 named agents** and **8,279 distinct issuer→recipient pairs** (8,237 edges after removing self-loops), with edge weight defined as the number of distinct tablets attesting the pair.

An earlier version of this analysis used only the flat path — 1,999 agents, 4,051 edges — and explained the modest coverage by saying that many genres "omit" attribution. That explanation was wrong, and the error is worth recording. The scribes did not omit the attribution on list-format tablets; they wrote it once at header or footer scope, and the pipeline's hierarchical path had already extracted it. The flat-only network was an analysis artifact — the join between record-level issuers and entry-level recipients was simply never made. Making it doubles the network and, as shown below, sharpens rather than dilutes every structural signature, which is itself informative: the list-format data the flat path dropped was not noise but the most institutionally structured part of the record.

### 9.2 Structure against a null model

Each statistic is tested against a degree-preserving null (directed configuration model, 100 randomizations): would a random network with exactly these agents and exactly these connection counts show the same feature?

**Table 4.** Combined-network statistics versus degree-preserving null (mean ± s.d. over 100 randomizations).

| Statistic | Observed | Null model | z-score |
|---|---|---|---|
| Agents / edges | 4,209 / 8,237 | — | — |
| Reciprocity | 0.052 | 0.022 ± 0.002 | +16.3 |
| Giant-component share | 84.7% | 90.9% ± 0.5% | −11.7 |
| Communities (modularity) | 69 | — | — |

**Reciprocity is low, but far above chance.** Only 5.2% of directed ties are reciprocated — grain overwhelmingly moves one way along any given relationship, as a redistributive hierarchy predicts and a marketplace of bilateral traders does not. Yet that figure is more than double the null expectation (z = +16.3): mutual ties are rare but systematically present, consistent with the documented role of senior officials who both receive allocations into their household accounts and disburse from them.

**The network is less globally connected than chance predicts.** The giant component spans 84.7% of agents, significantly below the null's 90.9% (z = −11.7). Degree structure alone would knit this network tighter than it actually is; the shortfall indicates compartmentalization — clusters that keep their connections internal. Modularity detection finds 69 communities, the two largest containing 1,059 and 1,009 agents, a granularity consistent with the corpus's known archive structure (Drehem, Umma, Girsu as separate administrative worlds with sparse cross-links).

**The royal estate emerges as the system's sink — but only in the combined network.** The highest in-degree node by a wide margin is *lugal* (the king, in context the royal estate), receiving from 669 distinct issuers — an institutional destination, not a person, and one the flat-path network had almost entirely missed, because grain reaches the crown through header-scoped list documents rather than bilateral receipts. This converges with the verified subset (Tier 2), where *lugal* independently appears as the dominant recipient (9.3% of verified grain volume, Section 8.2), and with the philological picture of centralized extraction (Steinkeller 1987). Below the crown, the hubs are the expected accountable officials: ba-zi disburses to 207 distinct recipients (consistent with the high-volume Drehem disbursement official of that name), ur-Baba receives from 88 distinct issuers, while more than half of all agents (2,432 of 4,209) have exactly one incoming source — the signature of ordinary workers drawing from a single office. Two independent methods — close reading and bulk extraction — arrive at the same institutional picture; given that Tier-1 attribution is unverified, convergence, not proof, is the correct word.
...
3. **Coverage is partial and non-random.** 42% of tablets yield transactions; even in the combined relational dataset, most extracted quantities carry no resolvable counterparty; 28.1% of transactions have a year. Every filtered view shrinks the effective sample and potentially skews it toward formula-rich genres.
4. **Attribution does not inherit across section boundaries.** Within an accounting section, the parser propagates issuer and recipient to every entry, and the hierarchical path captures header-scoped attribution over list items. But a tablet-final issuer clause governing several *šunigin*-closed groups above it is credited only to its own section. This is a genuine statefulness limitation of the current segmentation design; relational coverage is accordingly a lower bound, and the network of Section 9 under-represents multi-section summary accounts.
5. **Large samples do not cure systematic error.** Section 5 is the proof at the reference-data level, and Section 9.1 is the proof at the analysis level: 90,000 transactions carried a wrong king label because one lookup table and one default rule were wrong, and an entire relational layer went unused because one join was never made. Scale amplifies correlated errors; it does not average them away.
6. **The archive is not the economy.** Survival, excavation, and publication are all biased processes; informal and non-institutional exchange is invisible; even the recorded state economy is seen only through documents that happened to survive.
7. **The literature review is survey-depth, not exhaustive.** Related work (Section 1.1) was identified and characterized from titles, abstracts, and published summaries, not from full-text study, and network access constraints prevented reading the primary computational-Assyriology literature in full. Consequently this paper claims no priority for any technique — including the šunigin harness, whose absence from the surveyed material is not evidence of absence from the field.
8. **Residual extraction risk.** Idiosyncratic multi-level accounts may still leak double-counted totals past the balance filters; the conversion of *sila₃* to modern mass is contested and all tonnage figures are indicative only.
...
**A reader may take from this paper:** that the CDLI Ur III corpus supports transaction-level extraction at a scale of hundreds of thousands of records; that on an arithmetically verified subset, the modal grain allocation is exactly the 60-*sila₃* monthly ration known from prior scholarship, and recorded grain volume within that record type is highly concentrated across mixed transaction scales (Gini 0.873, with the constraints of Section 8.2); that at corpus scale, recorded cereal volume is on the order of 2.4 million *gur*, distributed log-normally with an extreme tail in which 1% of transactions carry 54% of volume; and that the attributed exchange network — once both bilateral and list-format relational data are combined — shows above-chance reciprocity, below-chance global cohesion, and the royal estate as its dominant sink: a compartmentalized, largely one-directional flow structure convergent with the redistributive model of the philological literature.

**A reader may not take from this paper:** that any individual unverified transaction is correct; that the Gini coefficient measures Ur III income or wealth inequality; that the temporal profile tracks economic activity rather than archival survival; or that any component of this work is unprecedented — the related work of Section 1.1 was surveyed at abstract depth only, and this paper claims usefulness, not priority, for its methods.
...
The extraction pipeline, its 184-test regression suite, the šunigin reconciliation harness, and the analysis scripts are available in the project repository (`asarodan/sumer`, branch `claude/previous-chat-recovery-dcsu9m`). All figures in this paper derive from a fixed corpus snapshot, cited here so that every count is reproducible against the exact input: CDLI Ur III bulk ATF export, downloaded 2026-06-19, 86,897,841 bytes, SHA-256 `948c2ba522fcbb9a387cde85de8a8362e3c3f4f1325bcf65e2de325259be26a6`, containing 135,199 tablets. The live CDLI database grows and is corrected continuously; running this pipeline against a later export will — and should — produce different counts. The verified 90-tablet dataset and the full transaction extraction accompany the digital exhibition as downloadable CSVs. The CDLI corpus is distributed by the Cuneiform Digital Library Initiative under its data-sharing terms.

## References

Anderson, A. G. (2017). *The Old Assyrian Social Network: An Analysis of the Texts from Kültepe-Kanesh (1950–1750 B.C.E.).* PhD dissertation, Harvard University.

Bamman, D., A. Anderson, and N. A. Smith (2013). "Inferring Social Rank in an Old Assyrian Trade Network." *Digital Humanities 2013*, conference abstract.

Chiarcos, C., É. Pagé-Perron, I. Khait, N. Schenk, and L. Reckling (2018). "Towards a Linked Open Data Edition of Sumerian Corpora." *Proceedings of the Eleventh International Conference on Language Resources and Evaluation (LREC 2018).*

Cuneiform Digital Library Initiative (CDLI). *Ur III administrative corpus, ATF bulk export.* cdli.mpiwg-berlin.mpg.de. Snapshot of 2026-06-19, SHA-256 948c2ba5…be26a6 (full hash in Data and Code Availability).
...
Jaworski, W. (2008). "Contents Modelling of Neo-Sumerian Ur III Economic Text Corpus." *Proceedings of the 22nd International Conference on Computational Linguistics (COLING 2008)*, 369–376.

Liu, C. (2021). *The Ur III Administrative Texts from Puzriš-Dagan Kept in the Harvard Museum of the Ancient Near East.* Harvard Semitic Studies 68. Leiden: Brill.
...
Pagé-Perron, É., M. Sukhareva, I. Khait, and C. Chiarcos (2017). "Machine Translation and Automated Analysis of the Sumerian Language." *Proceedings of the Joint SIGHUM Workshop on Computational Linguistics for Cultural Heritage, Social Sciences, Humanities and Literature (LaTeCH-CLfL 2017)*, 10–16.
...
Widell, M. (2003). *The Administrative and Economic Ur III Texts from the City of Ur.* Piscataway, NJ: Gorgias Press.
