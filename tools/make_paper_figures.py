...
plt.rcParams.update({'font.family':'serif','font.serif':['DejaVu Serif'],
    'font.size':10,'axes.titlesize':11,'axes.labelsize':10,'figure.dpi':130,
    'savefig.dpi':200,'axes.grid':True,'grid.alpha':0.25,'grid.linewidth':0.5,'axes.axisbelow':True})
INK='#1a1a1a';ACC='#8b1a1a';GOLD='#b8860b';STEEL='#2c4a6e'
FIG='output/figures/'
tx=pd.read_csv('output/transactions_live.csv')
s=tx[tx.unit=='sila3']
b=s[(s.commodity=='barley')&(s.quantity>0)]

# FIG1
fig,ax=plt.subplots(figsize=(6.5,4))
q=b.quantity.values;lq=np.log10(q)
...
ax.set_xlabel('Transaction size  (log₁₀ sila₃)');ax.set_ylabel('Number of transactions')
ax.set_title(f'(a)  Barley transaction-size distribution  (N = {len(q):,})')
ax.legend(frameon=False,fontsize=8.5);ax.set_xticks(range(0,8))
ax.set_xticklabels(['1','10','100','1k','10k','100k','1M','10M'])
plt.tight_layout();plt.savefig(FIG+'fig1_txsize.png');plt.close()

# FIG2
def lorenz(x):
    x=np.sort(x);c=np.cumsum(x)/x.sum();return np.insert(c,0,0)
def g(x):
    x=np.sort(x);n=len(x);i=np.arange(1,n+1);return (2*np.sum(i*x)/(n*np.sum(x)))-(n+1)/n
...
ax.set_yticks(range(len(by)));ax.set_yticklabels(by.index);ax.set_xscale('log')
ax.set_xlabel('Volume  (gur, log scale)');ax.set_title('(c)  Capacity-measured commodity volume')
for i,v in enumerate(by.values):ax.text(v*1.1,i,f'{v:,.0f}',va='center',fontsize=7.5,color=INK)
...
ax.set_xlabel('Šulgi regnal year');ax.set_ylabel('Barley transactions')
ax.set_title('(d)  Dated barley transactions across Šulgi’s reign')
ax.axvspan(44.5,48.5,color=GOLD,alpha=0.12)
ax.annotate('archive\npeak\n(yr 45–48)',xy=(46,yc.max()*0.85),fontsize=8,ha='center',color=INK)
...
ax.axvline(np.log10(10000),color=GOLD,ls=':',lw=1.4,label='10k gur (granary scale)')
ax.set_xlabel('Per-tablet barley total  (log₁₀ gur)');ax.set_ylabel('Number of tablets')
ax.set_title(f'(e)  Per-tablet barley totals  (N = {len(pt):,} tablets)')
ax.legend(frameon=False,fontsize=8.5);ax.set_xticks(range(-1,6))
ax.set_xticklabels(['0.1','1','10','100','1k','10k','100k'])
plt.tight_layout();plt.savefig(FIG+'fig5_pertablet.png');plt.close()
print("figs 1-5 regenerated from live data")
